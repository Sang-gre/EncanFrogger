...
public class LevelManager {
    ...
    // --- Spawn groups (for coordinated respawn) ---
    ...
    public void loadLevel(int level, GameMap map) {
        ...
        initLaneClassification();
        assignLaneTypesForMap(map);

        obstacleSpeed = BASE_SPEED + (level - 1) * SPEED_INCREMENT;
    ...
    private void repositionEntities() {
        ...
        for (Platform p : platforms) {
            ...
            int availH = Math.max(1, laneHeight - LANE_TOP_PADDING);
            Dimension d = scaleToFitLane(img, columnWidth * 2, availH);
            int y = centeredY(lane, laneHeight) + LANE_TOP_PADDING + (availH - d.height) / 2;
    ...
    public void update() {
        ...
        for (Map.Entry<Integer, List<List<Platform>>> entry : platformGroups.entrySet()) {
            for (List<Platform> group : entry.getValue()) {
                if (isPlatformGroupOffScreen(group))
    ...
    private void spawnObstacles() {
        for (int lane = 0; lane < LANE_COUNT; lane++) {
            ...
            String type = laneObstacleType[lane];
            ...
            for (int g = 0; g < groupsPerLane; g++) {
                ...
                int baseX = columnX[g * spacing];

                for (int j = 0; j < groupSize; j++) {
                    int x = baseX + j * (d.width + GROUP_GAP);
                    int y = centeredY(lane, d.height);

                    Obstacle o = new Obstacle(x, y, d.width, d.height, lane, obstacleSpeed, dir, type);
                    o.setImage(img);
    ...
    public void spawnPlatforms() {
        for (int lane = 0; lane < LANE_COUNT; lane++) {
            ...
            Direction dir = (lane % 2 == 0) ? Direction.RIGHT : Direction.LEFT;
            String type = lanePlatformType[lane];
            Image img = AssetManager.getInstance().getPlatformImage(type);
            int availH = Math.max(1, laneHeight - LANE_TOP_PADDING);
            Dimension d = scaleToFitLane(img, columnWidth * 2, availH);
            ...
            for (int g = 0; g < groupsPerLane; g++) {
                ...
                for (int j = 0; j < groupSize; j++) {
                    ...
                    Platform p = new Platform(x, y, d.width, d.height, lane, obstacleSpeed * 0.7f, dir, type);
                    p.setImage(img);
    ...
    public void spawnCoins() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            while (!placed && attempts < 15) {
                ...
                int x = columnX[col] + (columnWidth - COIN_SIZE) / 2;
                int y = centeredY(lane, COIN_SIZE);

                Coin coin = new Coin(x, y, COIN_SIZE, COIN_SIZE);
                coin.setScaledSize((int) (laneHeight * 0.5f));

                if (isPlatformLane[lane]) {
                    placed = tryPlaceCoinOnPlatform(coin, lane);
                } else {
                    placed = tryPlaceCoinInLane(coin);
                }
    ...
    private boolean tryPlaceCoinOnPlatform(Coin coin, int lane) {
        ...
        int cx = pf.getX() + pf.getWidth() / 2 - COIN_SIZE / 2;
        int cy = pf.getY() + pf.getHeight() / 2 - COIN_SIZE / 2;
    ...
    private boolean tryPlaceCoinInLane(Coin coin) {
    ...
    private void respawnObstacleGroup(List<Obstacle> group) {
        ...
        for (int i = 0; i < group.size(); i++) {
            int x = (dir == Direction.RIGHT)
                    ? baseX - i * (d.width + GROUP_GAP)
                    : baseX + i * (d.width + GROUP_GAP);

            group.get(i).reset(x, centeredY(lane, d.height), obstacleSpeed, dir);
            group.get(i).setSize(d.width, d.height);
            group.get(i).setImage(img);
        }
    }
    ...
    private void respawnPlatformGroup(List<Platform> group) {
        ...
        for (int i = 0; i < group.size(); i++) {
            ...
            group.get(i).reset(x, y, obstacleSpeed * 0.7f, dir);
            group.get(i).setSize(d.width, d.height);
            group.get(i).setImage(img);
        }
    }
    ...
    private void assignLaneTypesForMap(GameMap map) {
        ...
        for (int lane = 0; lane < LANE_COUNT; lane++) {
            if (isObstacleLane[lane] && obstacleTypes != null && obstacleTypes.length > 0)
                laneObstacleType[lane] = obstacleTypes[rng.nextInt(obstacleTypes.length)];

            if (isPlatformLane[lane] && platformTypes != null && platformTypes.length > 0)
                lanePlatformType[lane] = platformTypes[rng.nextInt(platformTypes.length)];
        }
    }

    private void initAssetPools() {
    ...
    private boolean isGroupOffScreen(List<Obstacle> group) {
    ...
    private boolean isPlatformGroupOffScreen(List<Platform> group) {
